...
def FindMatchBetweenBrainKeypoints(mKeySkullStrippedKeypoint,mKeyOriginalKeypoint,maxDistance=5000):
    ...
    f = 64
    tOri = AnnoyIndex(f, 'euclidean')  # Length of item vector that will be indexed
    for i in range(kOri.shape[0]):
        v = kOri[i,kI.descriptor]
        tOri.add_item(i, v)
    tOri.build(10) # 10 trees

    outputMat=np.zeros((0,2))
    for i in range(kSS.shape[0]):
        [idx1,dist1 ]=tOri.get_nns_by_vector(kSS[i,kI.descriptor],1,include_distances=True)
        idx=int(idx1[0])
        dist=dist1[0]
        if dist<maxDistance:
            a=np.array([[i,idx]])
